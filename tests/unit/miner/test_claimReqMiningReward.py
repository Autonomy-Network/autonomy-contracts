...
def test_claimReqMiningReward(auto, mockTarget, ethForCall, requester):
    if requester != auto.ALICE and requester != auto.DENICE:
        ...
        auto.m.claimReqMiningReward(1, {'from': requester})
        
        # Should've changed
        assert auto.AUTO.balanceOf(auto.m) == INIT_AUTO_REW_POOL - INIT_REQUESTER_REWARD
        for addr in addrs:
            assert auto.m.getAvailableMiningRewards(requester) == (0, 0, 0, 0)
            assert auto.AUTO.balanceOf(addr) - startBals[addr] == (INIT_REQUESTER_REWARD if addr == requester else 0)
...
def test_claimReqMiningReward_all_parties_random(auto, mockTarget, referer, ethForCall, requester, executor):
...
def test_claimReqMiningReward_rev_claimAmount(auto, sender, claimAmount):
    with reverts(REV_MSG_CLAIM_TOO_LARGE):
        auto.m.claimReqMiningReward(claimAmount, {'from': sender})