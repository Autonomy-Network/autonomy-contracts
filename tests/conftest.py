...
def cleanAUTO(AUTO, PriceOracle, Oracle, StakeManager, Registry, Forwarder, Miner, Timelock):
...
def hashedReqs(auto, mockTarget):
    ...
    reqs.append((auto.BOB.address, mockTarget.address, auto.DENICE, callData, msgValue, 0, False, False, False, False, False, ""))
    tx = auto.r.newReqPaySpecific(mockTarget, auto.DENICE, callData, 0, False, False, False, False, False, "", {'from': auto.BOB, 'value': msgValue})

    # Set a var on a target, sending ETH with the call, pay upfront
    callData = mockTarget.setXPay.encode_input(5)
    reqs.append((auto.BOB.address, mockTarget.address, auto.DENICE, callData, msgValue, ethForCall, False, False, False, False, False, ""))
    tx = auto.r.newReqPaySpecific(mockTarget, auto.DENICE, callData, ethForCall, False, False, False, False, False, "", {'from': auto.BOB, 'value': msgValue})

    # Set a var on a target, paying with AUTO after execution
    callData = mockTarget.setX.encode_input(5)
    reqs.append((auto.BOB.address, mockTarget.address, auto.DENICE, callData, 0, 0, False, False, True, False, False, ""))
    tx = auto.r.newReqPaySpecific(mockTarget, auto.DENICE, callData, 0, False, False, True, False, False, "", {'from': auto.BOB, 'value': 0})

    # Set a var on a target, sending ETH witht the call, paying with AUTO after execution
    callData = mockTarget.setXPay.encode_input(5)
    reqs.append((auto.BOB.address, mockTarget.address, auto.DENICE, callData, ethForCall, ethForCall, False, False, True, False, False, ""))
    tx = auto.r.newReqPaySpecific(mockTarget, auto.DENICE, callData, ethForCall, False, False, True, False, False, "", {'from': auto.BOB, 'value': ethForCall})
    ...
    reqs.append((auto.BOB.address, mockTarget.address, auto.DENICE, callData, ethForCall, ethForCall, True, False, True, False, False, ""))
    tx = auto.r.newReqPaySpecific(mockTarget, auto.DENICE, callData, ethForCall, True, False, True, False, False, "", {'from': auto.BOB, 'value': ethForCall})
    ...
    reqs.append((auto.BOB.address, mockTarget.address, auto.DENICE, callData, ethForCall, ethForCall, False, True, True, False, False, ""))
    tx = auto.r.newReqPaySpecific(mockTarget, auto.DENICE, callData, ethForCall, False, True, True, False, False, "", {'from': auto.BOB, 'value': ethForCall})
    ...
    reqs.append((auto.BOB.address, mockTarget.address, auto.DENICE, callData, 0, 0, True, True, False, False, False, ""))
    tx = auto.r.newReqPaySpecific(mockTarget, auto.DENICE, callData, 0, True, True, False, False, False, "", {'from': auto.BOB, 'value': 0})
    ...
    reqs.append((auto.BOB.address, mockTarget.address, auto.DENICE, callData, ethForCall, ethForCall, True, True, True, False, False, ""))
    tx = auto.r.newReqPaySpecific(mockTarget, auto.DENICE, callData, ethForCall, True, True, True, False, False, "", {'from': auto.BOB, 'value': ethForCall})

    # Set a var on a target, pay in AUTO, and have it be recurring
    callData = mockTarget.setX.encode_input(5)
    reqs.append((auto.BOB.address, mockTarget.address, auto.DENICE, callData, 0, 0, False, False, True, True, False, ""))
...
def vulnerableHashedReqs(auto, mockTarget, vulnerableRegistry, stakedMin):
    ...
    reqEthForCall = (auto.BOB.address, mockTarget.address, auto.DENICE, callData, msgValue, ethForCall, False, False, False, False, False, "")
    tx = vulnerableRegistry.newReqPaySpecific(mockTarget, auto.DENICE, callData, ethForCall, False, False, False, False, False, "", {'from': auto.BOB, 'value': msgValue})

    auto.AUTO.approve(vulnerableRegistry, MAX_TEST_STAKE, auto.FR_BOB)

    reqPayAUTOEthForCall = (auto.BOB.address, mockTarget.address, auto.DENICE, callData, ethForCall, ethForCall, False, False, True, False, False, "")
    tx = vulnerableRegistry.newReqPaySpecific(mockTarget, auto.DENICE, callData, ethForCall, False, False, True, False, False, "", {'from': auto.BOB, 'value': ethForCall})
