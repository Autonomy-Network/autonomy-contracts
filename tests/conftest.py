import pytest
from brownie import web3, chain, Contract
from consts import *
from utils import *
...
def deploy_initial_AUTO_contracts(AUTO, PriceOracle, Oracle, StakeManager, Registry, Forwarder, Miner, Timelock, TProxy, TProxyAdmin):
    class Context:
        pass
    ...
    # auto.AUTO = auto.DEPLOYER.deploy(AUTO, )
    ...
    TProxy.remove(auto.r_proxy)
    auto.r = Registry.at(auto.r_proxy.address, owner=auto.DEPLOYER)
...
def cleanAUTO(AUTO, PriceOracle, Oracle, StakeManager, Registry, Forwarder, Miner, Timelock, TProxy, TProxyAdmin):
    deploy_ERC1820()
    return deploy_initial_AUTO_contracts(AUTO, PriceOracle, Oracle, StakeManager, Registry, Forwarder, Miner, Timelock, TProxy, TProxyAdmin)
...
@pytest.fixture(scope="module")
def freshAUTOToken(AUTO):
    deploy_ERC1820()
    deployer = a[0]

    return deployer.deploy(AUTO, "Autonomy Network", "AUTN", [], deployer, INIT_AUTO_SUPPLY)
